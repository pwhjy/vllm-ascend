...
from .turboquant_runtime import build_qjl_projection, build_rotation_matrix, build_turboquant_codebook
...
class AscendTurboQuantKVCacheAttentionMethod(AscendAttentionScheme):
    def __init__(self, quant_description: dict, prefix: str):
        ...
        self.k_bits = int(tq_config.get("k_bits", 3))
        self.v_bits = int(tq_config.get("v_bits", 2))
    ...
    def create_weights(self, layer: torch.nn.Module) -> None:
        ...
        head_size = int(layer.impl.head_size)
        params_dtype = torch.float32
        seed = self.rotation_seed_base + self.layer_id

        k_codebook, k_boundary = build_turboquant_codebook(head_size, self.k_bits, "cpu", params_dtype)
        v_codebook, v_boundary = build_turboquant_codebook(head_size, self.v_bits, "cpu", params_dtype)
        k_rot = build_rotation_matrix(head_size, seed, "cpu", params_dtype)
        v_rot = build_rotation_matrix(head_size, seed + 1, "cpu", params_dtype)
        k_qjl_proj = build_qjl_projection(head_size, seed + 2, "cpu", params_dtype)
        ...
        layer.tq_k_bits = self.k_bits
        layer.tq_v_bits = self.v_bits
        ...
        layer.register_parameter("k_codebook", torch.nn.Parameter(k_codebook, requires_grad=False))
        layer.register_parameter("v_codebook", torch.nn.Parameter(v_codebook, requires_grad=False))
        layer.register_parameter("k_boundary", torch.nn.Parameter(k_boundary, requires_grad=False))
        layer.register_parameter("v_boundary", torch.nn.Parameter(v_boundary, requires_grad=False))
        layer.register_parameter("k_rot", torch.nn.Parameter(k_rot, requires_grad=False))
        layer.register_parameter("v_rot", torch.nn.Parameter(v_rot, requires_grad=False))
        layer.register_parameter("k_qjl_proj", torch.nn.Parameter(k_qjl_proj, requires_grad=False))
