...
BEGIN_TILING_DATA_DEF(TqEncodeProdPagedCacheTilingData)
    TILING_DATA_FIELD_DEF(uint32_t, totalTokens);
    TILING_DATA_FIELD_DEF(uint32_t, numKvHeads);
    TILING_DATA_FIELD_DEF(uint32_t, blockSize);
    TILING_DATA_FIELD_DEF(uint32_t, idxPackedCols);
    TILING_DATA_FIELD_DEF(uint32_t, qjlPackedCols);
...
REGISTER_TILING_DATA_CLASS(TqEncodeProdPagedCache, TqEncodeProdPagedCacheTilingData)
