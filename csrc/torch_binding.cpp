...
void check_tq_encode_common_inputs(
    const at::Tensor& x,
    ...
    int64_t head_dim)
{
    TORCH_CHECK(
        ...
        "x must be fp32/fp16/bf16");
    TORCH_CHECK(slot_mapping.scalar_type() == at::kLong,
                "slot_mapping must be int64");
    TORCH_CHECK(idx_cache.scalar_type() == at::kByte,
                "idx_cache must be uint8");
    TORCH_CHECK(norm_cache.scalar_type() == at::kFloat,
                "norm_cache must be fp32");
    TORCH_CHECK(rotation.scalar_type() == at::kFloat,
                "rotation must be fp32");
    TORCH_CHECK(boundary.scalar_type() == at::kFloat,
                "boundary must be fp32");
    TORCH_CHECK(x.dim() == 3, "x must be [num_tokens, num_kv_heads, head_dim]");
    TORCH_CHECK(slot_mapping.dim() == 1, "slot_mapping must be 1D");
    TORCH_CHECK(idx_cache.dim() == 4,
                "idx_cache must be [num_blocks, block_size, num_kv_heads, packed_cols]");
    TORCH_CHECK(norm_cache.dim() == 4,
                "norm_cache must be [num_blocks, block_size, num_kv_heads, 1]");
    TORCH_CHECK(rotation.dim() == 2, "rotation must be [head_dim, head_dim]");
    TORCH_CHECK(boundary.dim() == 1, "boundary must be 1D");
    TORCH_CHECK(slot_mapping.size(0) >= x.size(0),
                "slot_mapping must cover all input tokens");
    TORCH_CHECK(idx_cache.size(2) == x.size(1),
                "idx_cache num_kv_heads must match x");
    TORCH_CHECK(norm_cache.size(2) == x.size(1),
                "norm_cache num_kv_heads must match x");
    TORCH_CHECK(norm_cache.size(3) == 1, "norm_cache last dim must be 1");
    TORCH_CHECK(x.size(2) == head_dim, "x last dim must match head_dim");
    TORCH_CHECK(rotation.size(0) == head_dim && rotation.size(1) == head_dim,
                "rotation shape must match head_dim");
    TORCH_CHECK(bits >= 1 && bits <= 4, "bits must be in [1, 4]");
    TORCH_CHECK(head_dim > 0 && head_dim <= 256,
                "head_dim must be in (0, 256] for the first P6 encode kernel");
    int64_t packed_cols = (head_dim * bits + 7) / 8;
    TORCH_CHECK(idx_cache.size(3) >= packed_cols,
                "idx_cache packed cols are too small for head_dim/bits");
    TORCH_CHECK(packed_cols <= 128,
                "packed cols must be <= 128 for the first P6 encode kernel");
}

}  // namespace

void tq_encode_mse_paged_cache(
    const at::Tensor& x,
    const at::Tensor& slot_mapping,
    const at::Tensor& idx_cache,
    const at::Tensor& norm_cache,
    ...
    int64_t head_dim)
{
    ...
    TORCH_CHECK(qjl_cache.scalar_type() == at::kByte,
                "qjl_cache must be uint8");
    TORCH_CHECK(gamma_cache.scalar_type() == at::kFloat,
                "gamma_cache must be fp32");
    TORCH_CHECK(codebook.scalar_type() == at::kFloat,
                "codebook must be fp32");
    TORCH_CHECK(qjl_proj_t.scalar_type() == at::kFloat,
                "qjl_proj_t must be fp32");
    TORCH_CHECK(qjl_cache.dim() == 4,
                "qjl_cache must be [num_blocks, block_size, num_kv_heads, packed_cols]");
    TORCH_CHECK(gamma_cache.dim() == 4,
                "gamma_cache must be [num_blocks, block_size, num_kv_heads, 1]");
    TORCH_CHECK(codebook.dim() == 1, "codebook must be 1D");
    TORCH_CHECK(qjl_proj_t.dim() == 2,
                "qjl_proj_t must be [head_dim, head_dim]");
    TORCH_CHECK(qjl_cache.size(2) == x.size(1),
                "qjl_cache num_kv_heads must match x");
    TORCH_CHECK(gamma_cache.size(2) == x.size(1),
                "gamma_cache num_kv_heads must match x");
    TORCH_CHECK(gamma_cache.size(3) == 1, "gamma_cache last dim must be 1");
    TORCH_CHECK(qjl_proj_t.size(0) == head_dim && qjl_proj_t.size(1) == head_dim,
                "qjl_proj_t shape must match head_dim");
    TORCH_CHECK(total_bits >= 2 && total_bits <= 5,
                "prod total_bits must be in [2, 5]");
    int64_t qjl_cols = (head_dim + 7) / 8;
    TORCH_CHECK(qjl_cache.size(3) >= qjl_cols,
                "qjl_cache packed cols are too small for head_dim");
...
at::Tensor tq_prod_paged_k_score(
    ...
    int64_t max_seq_len)
{
    ...
    TORCH_CHECK(norm.scalar_type() == at::kFloat, "norm must be fp32");
    ...
    TORCH_CHECK(codebook.scalar_type() == at::kFloat, "codebook must be fp32");
    TORCH_CHECK(q_rot.dim() == 3, "q_rot must be [batch, num_heads, head_dim]");
    TORCH_CHECK(q_qjl.sizes() == q_rot.sizes(), "q_qjl must have the same shape as q_rot");
    TORCH_CHECK(packed_idx.dim() == 4,
                "packed_idx must be [num_blocks, block_size, num_kv_heads, packed_cols]");
    TORCH_CHECK(packed_qjl.dim() == 4,
                "packed_qjl must be [num_blocks, block_size, num_kv_heads, qjl_cols]");
    TORCH_CHECK(gamma.dim() == 4, "gamma must be [num_blocks, block_size, num_kv_heads, 1]");
    TORCH_CHECK(norm.dim() == 4, "norm must be [num_blocks, block_size, num_kv_heads, 1]");
    TORCH_CHECK(block_table.dim() == 2, "block_table must be [batch, max_blocks_per_seq]");
    TORCH_CHECK(seq_lens.dim() == 1, "seq_lens must be [batch]");
    TORCH_CHECK(q_rot.size(0) == block_table.size(0), "q_rot batch must match block_table");
    TORCH_CHECK(q_rot.size(0) == seq_lens.size(0), "q_rot batch must match seq_lens");
    TORCH_CHECK(q_rot.size(1) % packed_idx.size(2) == 0,
                "num_heads must be divisible by num_kv_heads");
    TORCH_CHECK(q_rot.size(2) == head_dim, "q_rot last dim must match head_dim");
    TORCH_CHECK(total_bits >= 2 && total_bits <= 5, "prod total_bits must be in [2, 5]");
    ...
    at::Tensor out = at::empty(
        ...
        q_rot.options().dtype(at::kFloat));

    EXEC_NPU_CMD(aclnnTqProdPagedKScore,
        q_rot, q_qjl, packed_idx, packed_qjl, gamma, norm,
        block_table, seq_lens, codebook,
        total_bits, head_dim, scale, max_seq_len, out);

    return out;
}

at::Tensor tq_prod_mse_paged_attention(
    const at::Tensor& q_rot,
    const at::Tensor& q_qjl,
    ...
    int64_t score_tile_len)
{
    ...
    TORCH_CHECK(q_rot.dim() == 3, "q_rot must be [batch, num_heads, head_dim]");
    TORCH_CHECK(q_qjl.sizes() == q_rot.sizes(),
                "q_qjl must have the same shape as q_rot");
    ...
    TORCH_CHECK(q_rot.size(0) == block_table.size(0),
                "q_rot batch must match block_table");
    TORCH_CHECK(q_rot.size(0) == seq_lens.size(0),
                "q_rot batch must match seq_lens");
    TORCH_CHECK(q_rot.size(1) % k_packed_idx.size(2) == 0,
                "num_heads must be divisible by num_kv_heads");
    TORCH_CHECK(q_rot.size(2) == head_dim,
                "q_rot last dim must match head_dim");
    ...
    EXEC_NPU_CMD(aclnnTqProdMsePagedAttention,
    ...
    int64_t pretransformed_query)
{
    ...
    TORCH_CHECK(k_qjl_proj_t.scalar_type() == at::kFloat,
                "k_qjl_proj_t must be fp32");
    TORCH_CHECK(k_boundary.scalar_type() == at::kFloat,
                "k_boundary must be fp32");
    ...
    TORCH_CHECK(v_boundary.scalar_type() == at::kFloat,
                "v_boundary must be fp32");
    TORCH_CHECK(k_codebook.scalar_type() == at::kFloat,
                "k_codebook must be fp32");
    TORCH_CHECK(v_codebook.scalar_type() == at::kFloat,
                "v_codebook must be fp32");
    ...
    TORCH_CHECK(slot_mapping.dim() == 1, "slot_mapping must be [batch]");
    TORCH_CHECK(k_packed_idx.dim() == 4,
                "k_packed_idx must be [num_blocks, block_size, num_kv_heads, packed_cols]");
    TORCH_CHECK(k_packed_qjl.dim() == 4,
                "k_packed_qjl must be [num_blocks, block_size, num_kv_heads, qjl_cols]");
    ...
    TORCH_CHECK(query.size(0) == slot_mapping.size(0),
                "query batch must match slot_mapping");
    ...
    TORCH_CHECK(k_boundary.numel() == ((1LL << (k_total_bits - 1)) - 1),
                "k_boundary size must match k stage1 bits");
    TORCH_CHECK(v_boundary.numel() == ((1LL << v_bits) - 1),
                "v_boundary size must match v_bits");
    TORCH_CHECK(head_dim > 0 && head_dim <= 256,
...
TORCH_LIBRARY_EXPAND(CONCAT(_C, _ascend), ops)
{
    ...
    ops.impl("tq_encode_mse_paged_cache", torch::kPrivateUse1,
    ...
    ops.def(
        "tq_prod_paged_k_score(Tensor q_rot, Tensor q_qjl, "
        "                      Tensor packed_idx, Tensor packed_qjl, "
        "                      Tensor gamma, Tensor norm, "
        "                      Tensor block_table, Tensor seq_lens, "
        "                      Tensor codebook, int total_bits, int head_dim, "
        "                      float scale, int max_seq_len) -> Tensor"
    );
    ops.impl("tq_prod_paged_k_score", torch::kPrivateUse1, &vllm_ascend::tq_prod_paged_k_score);
    ops.def(
        "tq_prod_mse_paged_attention(Tensor q_rot, Tensor q_qjl, "
